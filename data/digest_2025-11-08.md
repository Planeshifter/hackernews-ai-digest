...
#### [Submission URL](https://softwarepreservation.computerhistory.org/LISP/MIT/AIM-001.pdf) | 94 points | by [swatson741](https://news.ycombinator.com/user?id=swatson741) | [11 comments](https://news.ycombinator.com/item?id=45857045)

I can’t read raw PDF bytes from this paste. Please share a link to the Hacker News post or the article/PDF itself (or paste the text), and I’ll summarize it. If you only have the HN title, that works too—I can pull context from the discussion page.

The Hacker News discussion explores the historical origins and technical underpinnings of LISP's foundational concepts, particularly **CAR** and **CDR**, and their ties to early computing hardware. Key points include:

1. **CAR/CDR Origins**:  
   - The terms **CAR** (Contents of Address Register) and **CDR** (Contents of Decrement Register) stem from the **IBM 704/709** architecture, where 36-bit machine words were split into 15-bit address/decrement parts. These hardware-specific terms became abstracted in LISP for list processing, with `CAR` accessing the head (first element) of a list and `CDR` referencing the remainder (tail).

2. **Hardware Influence**:  
   - Early LISP implementations mapped linked lists to the IBM 704’s memory structure, using address/decrement registers to navigate list elements. This design enabled efficient recursive processing and became a hallmark of LISP’s elegance despite its machine-specific roots.

3. **Recursive Advantages**:  
   - CAR/CDR’s structure facilitated **recursive algorithms**, allowing programmers to decompose lists elegantly. This abstraction elevated LISP above contemporaries like FORTRAN and COBOL, which lacked native list-processing primitives.

4. **Historical Context**:  
   - References to McCarthy’s early AIM reports (1958–1959) and Herbert Stoyan’s historical analyses highlight LISP’s evolution. McCarthy’s work abstracted machine-level details into a high-level language, while Stoyan’s writings and Hong-Yi Dai’s *"The Mysteries of Lisp"* (2015) dissect its enduring legacy and unresolved complexities.

5. **Modern Reimplementations**:  
   - Projects like **lispxv** (a subset LISP implementation) aim to recreate early LISP variants, emphasizing its minimalist design and historical significance.

The discussion underscores how LISP’s design—rooted in 1950s hardware—laid the groundwork for symbolic computation and functional programming, with contributors reflecting on its historical documents, technical choices, and ongoing relevance.
...
Title: How to turn off Firefox’s AI/LLM features (and why some users are upset)
