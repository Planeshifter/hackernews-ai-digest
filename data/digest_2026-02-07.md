...
#### [Submission URL](https://rlhfbook.com/) | 128 points | by [onurkanbkrc](https://news.ycombinator.com/user?id=onurkanbkrc) | [5 comments](https://news.ycombinator.com/item?id=46923463)

Headline: Living RLHF book gets a big reorg and is heading to v2

- Active, fast-moving project: Last built Feb 7, 2026, with steady updates since early 2025.
- Major reorg: In Jan 2026 the chapters were restructured to match the forthcoming Manning book; old URLs now redirect.
- Version milestones: v0 (mid‑Apr 2025) covered overoptimization and open questions; v1.1 (Jun 6, 2025) brought significant RLVR/reasoning improvements; v2 is in progress (Dec 2025) based on editor feedback.
- Breadth of content: Added chapters and deep dives on PPO and GAE, reward modeling and preference data, instruction fine-tuning (IFT), DPO (started late Feb, finished mid‑Mar 2025), evaluation, reasoning, and a tool-use chapter (Jul 2, 2025). Also touches “RLHF × Product,” site cleanup, and SEO.
- Dev notes: Lots of polishing and refactoring along the way; acknowledgements span contributors across the RL community (e.g., Costa Huang, John Schulman, Shane Gu) and GitHub helpers.

Why it matters: It’s becoming a comprehensive, practitioner-friendly guide to modern RLHF and LLM alignment techniques, now aligning with a publisher’s structure while keeping the web version alive and well-maintained.

**Discussion Summary:**

The conversation focused largely on housekeeping and community engagement regarding the project. Moderators updated the submission URL from the ArXiv PDF to the direct website (`rlhfbook.com`) following a user request. Commenters noted that the author is actively seeking input on the text, sparking a meta-joke that he is personally engaging in "learning from human feedback." A link to a previous discussion from February 2025 was also shared for additional context.
...
Software Engineering Is Back: Coding agents over frameworks
...
Brendan Gregg joins OpenAI to drive ChatGPT performance, calling AI datacenter efficiency a planetary imperative
...
#### [Submission URL](https://app.writtte.com/read/kZ8Kj6R) | 7 points | by [lasgawe](https://news.ycombinator.com/user?id=lasgawe) | [3 comments](https://news.ycombinator.com/item?id=46925443)

I’m missing the submission details. Please share the Hacker News link (or the article text), and I’ll write an engaging summary. If you want, I can also include:
- 3–5 key takeaways
- Notable HN comment themes
- Why it matters and who should read it
- Estimated read time

Any length or tone preference?

Based on the discussion provided, here is the daily digest summary for the submission.

**Topic:** Implementing precise credit systems for AI token usage and the complexities of LLM pricing models.

**The Discussion:**
The conversation centers on the technical and economic challenges of billing for Large Language Models (LLMs).

*   **Handling Floating-Point Precision:** The submission author (**lsgw**) highlights a technical approach to converting AI token usage into credits. The core insight involves using **multi-layered integer scaling** rather than standard floating-point arithmetic. This ensures financial precision is maintained even when dealing with millions of transactions based on per-million-token pricing models.
*   **The "Token" Frustration:** User **ltchky** critiques current pricing models, comparing them to complicated "gym memberships" or abstract SaaS points that favor providers over users. They argue that token pricing creates stress because it disconnects the cost from the actual resource consumed (compute).
*   **Compute vs. I/O:** The discussion concludes with consensus that the industry struggles to delineate specific compute resources per user. **lsgw** agrees that shifting from an I/O (token) model to a "compute minutes" or credit-based system would better support application developers trying to build complex, cost-heavy applications.
