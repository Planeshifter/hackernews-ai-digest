...
#### [Submission URL](https://github.com/rsionnach/sloppylint) | 15 points | by [kyub](https://news.ycombinator.com/user?id=kyub) | [3 comments](https://news.ycombinator.com/item?id=46167703)

Sloppylint: an “AI slop” detector for Python code

What it is
- A linter focused on AI-generated code pitfalls that traditional tools often miss. It scans Python codebases for hallucinations, cross-language leakage, placeholder code, over-engineering, dead code, and more.

Why it exists
- Standard linters catch style and syntax, but AI output has new failure modes: invented imports, JS/Java idioms sneaking into Python, confident-yet-wrong implementations, and functions that “pass” or do nothing.

What it catches
- Hallucinated imports and symbols that don’t exist.
- Cross-language patterns (e.g., .push(), .length, .equals()).
- Placeholder code and hedging comments (e.g., pass, TODO, “should work hopefully”).
- Structural anti-patterns: bare except, star imports, single-method classes.
- Dead code, unused imports, copy-paste/duplicate code, over-engineering and deep nesting.
- Classic Python gotchas like mutable default args.

How it reports
- Severity buckets: Critical / High / Medium / Low.
- A “Slop Score” across axes: Information Utility (noise), Information Quality (lies), Style/Taste (soul), Structural Issues.
- Verdict tiers: CLEAN / ACCEPTABLE / SLOPPY / DISASTER.
- Optional JSON output for CI/CD.

CLI basics
- pip install sloppylint
- sloppylint . to scan a project.
- Flags: --severity high or --lenient (high+critical), --strict (everything), --ci, --max-score N, --ignore patterns, --disable check_name, --output report.json.
- Config via pyproject.toml.

Examples it flags
- mutable_default_arg (shared list/dict between calls).
- bare_except (swallows SystemExit/KeyboardInterrupt).
- pass_placeholder (unimplemented functions).
- hedging_comment (“should work hopefully”).

Notable claims
- Detects 100+ cross-language patterns.
- Catches “almost right” AI code issues before production (e.g., non-existent imports, wrong-language APIs).

Who it’s for
- Teams using AI assistants or reviewing AI-suggested patches who want a safety net beyond Pylint/Flake8, with CI-friendly gating on a “Slop Score.”

**Sloppylint: an “AI slop” detector for Python code**

**The Submission:**
Sloppylint is a new linter designed to catch errors common in AI-generated code that standard tools often miss, such as hallucinated imports, cross-language syntax leakage (like Java idioms in Python), and "hedging" comments. It assigns a "Slop Score" based on information utility and quality, aimed at teams using AI assistants who need a safety net for "almost right" code.

**The Discussion:**
Commenters discussed the overlap between Sloppylint and the current ecosystem, with one user noting that standard linters already catch issues like mutable default arguments. There was also a specific request to add **pre-commit** support to make the tool easier to integrate into development pipelines.
