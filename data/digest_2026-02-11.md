...
#### [Submission URL](https://z.ai/blog/glm-5) | 448 points | by [CuriouslyC](https://news.ycombinator.com/user?id=CuriouslyC) | [505 comments](https://news.ycombinator.com/item?id=46974853)

Please share the Hacker News submission you want summarized. You can provide:
- The HN thread link (or a screenshot)
- The source article URL or pasted text
- Any notable comments you want included
- Your preferred length or tone (e.g., punchy, technical, non-technical)

I’ll return an engaging digest with: a sharp headline, a concise summary, why it matters, and standout HN chatter.

Based on the scrambled comment data provided, it appears there are actually **two distinct hot topics** merged in your text: a technical discussion about a new RL training framework (**Slime**) and a cultural discussion about a new "vibes" benchmark (**Pelican riding a bicycle**).

Here is the digest covering the discussion around these topics.

***

### 1. The Technical Deep Dive: THUDM’s "Slime"
**The Story:** A new open-source framework called **Slime** (by THUDM) has been released to tackle the infrastructure challenges of Reinforcement Learning (RL) training, specifically aiming to democratize the "post-training" capabilities seen in models like DeepSeek-R1.

**Why it Matters:** As the industry pivoting toward reasoning models (like o1 and R1), the bottleneck isn't just the base model—it's the **RL infrastructure**. Training these models requires massive scale, efficient "rollouts" (generating answers to learn from), and complex synchronization. Slime attempts to solve the "systems problem" of RL, where 90% of the time is spent just waiting for GPUs to generate text.

**Standout HN Chatter:**
*   **The Hidden Bottleneck:** User `jfaganel99` highlights that the gap between frontier and non-frontier models is increasingly about RL infrastructure. They note that rollout generation takes up 90%+ of training time, making efficient handling of "stragglers" (slow GPUs) and long-tail token generation critical.
*   **Why is it hard?** When user `yvdm` asks why this is difficult, `pm` provides a breakdown: Unlike standard training, RL involves generating massive amounts of text across thousands of inference servers, scoring them, and updating weights synchronously. If one GPU hangs, everyone waits.
*   **The Solution:** Commenters discuss "APRIL," a strategy linked to Slime that handles these long-tail problems by decoupling the workers that generate text (rollout) from the ones that train the model, preventing system-wide stalls.

***

### 2. The Community Benchmark: The Pelican & The Bicycle
**The Story:** Simon Willison (`smnw`) shared a "vibes-based" benchmark where he asks LLMs to generate an SVG code file of a "pelican sitting on a bicycle." The results vary wildly, from abstract blobs to surprisingly accurate anatomical sketches.

**Why it Matters:** Traditional benchmarks (MMLU, SWE-bench) are becoming saturated or memorized by models. The community is turning to **creative coding tasks** (like generating SVGs) to test a model's "world model," spatial reasoning, and ability to handle vague instructions without hallucinations.

**Standout HN Chatter:**
*   **The "Vibe" Check:** User `ljm` defends the test, arguing that while it isn't scientific, it effectively tests how a model interprets ambiguity. It’s a measure of "practical engagement"—can the model figure out *how* a pelican would sit on a bike?
*   **Anatomy vs. Fantasy:** `hsprd` puts it humorously: "That's a bike that is ergonomically designed for pelicans. It is unreasonable to expect pelicans to ride human bikes."
*   **The Contamination Problem:** `TZubiri` and `slrzd` point out the catch-22 of modern benchmarking: By discussing the "Pelican on a Bicycle" test on Hacker News, the text and solution are now public. Future models will scrape this thread, include it in their training data, and the benchmark will become useless (Goodhart's Law).
*   **AGI Reality Check:** `RC_ITR` notes that models solving math Olympiad problems (AIME) but failing to draw a bird reveals the gap between "Artificial General Intelligence" and what we actually have: "Artificial Niche Intelligence."
