...
#### [Submission URL](https://github.com/davegoldblatt/total-recall) | 9 points | by [davegoldblatt](https://news.ycombinator.com/user?id=davegoldblatt) | [5 comments](https://news.ycombinator.com/item?id=46907183)

Total Recall: a write‑gated, tiered memory plugin for Claude Code
GitHub: davegoldblatt/total-recall

What it is
- A curated persistent memory system for Claude Code that avoids auto‑ingesting everything. It filters aggressively, logs first, and only promotes to long‑term memory on user command.

Why it matters
- Many AI “memory” tools bloat context with stale notes, increasing drift and hallucinations. Total Recall’s strict write gate and deterministic loading aim to keep memory lean, relevant, and trustworthy.

How it works
- Five-point write gate: save only if it changes future behavior, is a commitment, records a decision with rationale, is a durable/stable fact, or the user explicitly says “remember this.”
- Four tiers:
  - Working Memory (CLAUDE.local.md, ~1500 words): auto-loaded essentials.
  - Registers (structured files): searched on demand.
  - Daily Logs (timestamped capture): checked at session start; all writes land here first.
  - Archive: searchable history, never auto-loaded.
- Promotion is manual via /recall-promote to prevent premature conclusions.
- Corrections propagate immediately to log, registers, and working memory; contradictions are marked as superseded with dates/reasons.
- Optional “recall nudges” lightly suggest memory candidates (off by default).

Commands (namespaced if installed as a plugin)
- recall-init, recall-write (with gate), recall-log, recall-search, recall-promote, recall-status, recall-maintain, recall-forget, recall-context.

Install
- As a Claude Code plugin via its marketplace, or standalone by cloning into a project’s .claude/ directory.

Caveats
- Geared to Claude Code users; relies on manual promotion and a project file structure, which some workflows may not want.

Discussion centers on the philosophy of AI memory and configuration best practices.

**Key themes:**
*   **Curated vs. Raw Memory:** One user validates the "write gate" approach, arguing that treating memory as a curated database of decisions—rather than a raw log of tokens—is the correct heuristic for turning stateless assistants into persistent partners.
*   **Privacy Configuration:** A user suggested that the memory directory should be added to `.gitignore` by default to prevent accidental commits of personal contexts.
*   **Author Response:** The author (@dvgldbltt) agreed, immediately pushing an update to the installer to ignore memory files and local settings by default. They also noted plans to document a "team mode" that would allow committing shared decisions while keeping daily logs local.
*   **Performance:** Early feedback confirms the plugin operates as intended in daily workflows.

### Show HN: Pipeline and datasets for data-centric AI on real-world floor plans

#### [Submission URL](https://archilyse.standfest.science) | 11 points | by [standfest](https://news.ycombinator.com/user?id=standfest) | [4 comments](https://news.ycombinator.com/item?id=46896474)

I’m ready—please share the Hacker News submission you want summarized (URL or title), and, if you have it, the article link. I can also work from a screenshot or pasted text.

Preferences I can follow:
- Length: ultra-brief (50–80 words), standard (120–180), or deep dive (250–350)
- Angle: neutral, punchy, or technical
- Extras: notable comments, why it matters, key numbers, links

If you drop multiple HN links, I’ll turn them into a cohesive daily digest.

Here is a daily digest summary of the discussion provided:

**Topic:** Data-Centric AI & Floor Plan Analysis
**Length:** Standard (170 words)

This discussion highlights floor plan digitization as a prime example of **Data-Centric AI** winning over model-centric approaches. Commenters and the author (`stndfst`) argue that "adversarial" inputs—such as 100-year-old hand-drawn blueprints and low-res scans—break standard computer vision models. The consensus is that meaningful gains come from cleaning data (enforcing geometric consistency, scaling, and fixing labels) rather than swapping architectures (e.g., U-Net vs. Transformer).

The author detailed their approach to solving the "bottleneck of annotation quality": instead of relying on zero-shot magic, they built a baseline using **manual CAD-style annotation tools** that enforce strict constraints (closed rooms, attached walls) and rigorous QA gates.

**Why it matters:** The thread touches on a critical limitation in current Generative AI. As `V_Shukla` notes, model capabilities are commoditized, making data curation the real differentiator. The author warns that while generative models can create "pretty pictures" of floor plans, they often fail at preserving the **spatial topology and constraints** required for downstream tasks like energy modeling or digital twins.
