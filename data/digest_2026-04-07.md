...
#### [Submission URL](https://z.ai/blog/glm-5.1) | 596 points | by [zixuanlimit](https://news.ycombinator.com/user?id=zixuanlimit) | [243 comments](https://news.ycombinator.com/item?id=47677853)

Got it—send me the Hacker News submission you want summarized. Please include:
- HN link (or the item ID) and, if possible, the article URL
- Any text/content to summarize (article text or key points). I can’t browse, so paste what you want me to use.

Optional:
- Preferred length (blurb, 3–5 bullets, or 1–2 short paragraphs)
- Tone (neutral, punchy, or opinionated)
- Whether to include “Why it matters,” notable HN comments, and key numbers

I’ll turn it into a crisp, engaging digest entry.

Here is a crisp, punchy digest entry summarizing the Hacker News discussion.

**GLM-5.1 Drops: The 754-Billion Parameter Behemoth Sparking the "Local vs. Frontier" Debate**

A massive new open-weights model, GLM-5.1, has hit the scene, setting off a fierce debate on Hacker News about benchmarking, hardware constraints, and whether the moats protecting OpenAI and Anthropic are finally evaporating. While the model boasts frontier-level benchmark metrics, real-world user experiences are polarizing. 

**Key Takeaways from the HN Thread:**
*   **Highly Polarizing Performance:** Users are completely split on generation quality. Some claim GLM-5.1 rivals Claude Opus and Codex for complex, long-horizon coding tasks (like writing F# backends). Others complain it fails at basic PDF extraction, hallucinates dates, and struggles with context limits compared to commercial peers.
*   **Nostalgia for "Overnight Compiling":** The Unsloth quantization is a massive 361GB for a 754B parameter model. Unless you have top-tier hardware, running it locally is tough. However, hackers are discussing "SSD offloading" for complex code generation, joking that letting an AI run overnight to build project scaffolding feels exactly like the old days of compiling Linux kernels while you sleep.
*   **Are OpenAI and Anthropic "Cooked"?** Some commenters argue that open-weights and Chinese models (like Qwen and GLM) have officially closed the gap, claiming local/private inference is the future. Skeptics pushed back hard, calling this "bait" and pointing out that developers driving real economic value are still overwhelmingly paying for API access to OpenAI/Anthropic SOTA models.
*   **The Pelican Benchmark:** Developer Simon Willison used the model to generate a realistic animated pelican, which HN commenters are jokingly adopting as the new gold-standard benchmark. 

**Why it matters:** 
The open-source AI ecosystem is brute-forcing its way to frontier-level capabilities, but the sheer size of these new models introduces severe infrastructure hurdles. While the models are technically "open," actually running them requires either massive localized hardware or relying on APIs (where users suspect Huawei chips are currently bottlenecking GLM's inference times). We are entering an era where open models are smart enough to do the job, but heavy enough to break your hardware.
...
Anthropic: Assessing Claude Mythos Preview’s cybersecurity capabilities (Project Glasswing)
...
AI may be making us think and write more alike, USC researchers warn
...
#### [Submission URL](https://twitter.com/TheBobPony/status/2041112541909205001) | 115 points | by [bundie](https://news.ycombinator.com/user?id=bundie) | [71 comments](https://news.ycombinator.com/item?id=47674234)

Got it—please share the Hacker News submission you want summarized. You can provide:
- The HN link or item ID
- The title and article link
- Or paste the article text

Optional: tell me your preferred length (blurb, medium, deep dive) and audience (general/technical). I’ll return:
- TL;DR (1–2 sentences)
- Key points (3–5 bullets)
- Why it matters
- Link back to HN and the source article

Here is a summary of the Hacker News discussion regarding Microsoft’s new Copilot app for Windows. 

**(Note: The provided text was heavily compressed/vowel-stripped, but the underlying context points to a viral realization about how the Copilot Windows app is packaged, sparking a wider debate on Microsoft's AI integrations).**

### **TL;DR**
Hacker News users are roasting Microsoft after discovering that the "new" native Windows Copilot app is literally just a duplicated, renamed 850MB copy of the Microsoft Edge browser in a trenchcoat. The technical absurdity of this sparked a broader discussion about Copilot's fragile integrations and failure to execute basic enterprise tasks within Microsoft's own ecosystem.

### **Key Points**
*   **The Missing Native App:** Instead of building a native application or leveraging their own shared-runtime 'WebView2' framework (which Teams uses to save space), Microsoft simply duplicated the entire Chromium-based Edge browser into a subfolder. 
*   **A Horrible Case of Bloat:** Commenters noted the irony by comparing it to MS-DOS days (where `EDIT.COM` was just a tiny switch that launched `QBASIC.EXE` to save disk space) or Linux's `busybox`. Historically, these executable tricks were used to *conserve* storage; Microsoft's modern version just eats almost a gigabyte of redundant space.
*   **"Hobbled" Office Integrations:** Users vented severe frustration with Copilot's lack of true capability in M365. One user noted Copilot couldn't even perform a basic calendar overlap check for meetings because the underlying API limits it to reading a default of 10 attendees—proving it has an "impoverished view of the world."
*   **Performative AI vs. Actual Productivity:** The general consensus in the thread is that Microsoft is pushing "performative productivity" for executives to buy into, while the actual tools remain incoherent, fragile (often requiring enterprise Entra ID workarounds), and largely inferior to Anthropic's Claude or OpenAI's ChatGPT for getting actual work done.

### **Why It Matters**
This discussion highlights the growing friction between the massive AI hype-cycle and the reality of enterprise software development. When a nearly three-trillion-dollar company ships heavily marketed, flagship AI software as a bloated, duplicated web browser that struggles to read its own Outlook calendars, it signals to the developer community that current desktop AI integrations are still very much in a messy, "minimum viable product" phase. 

*No direct HN or article link was provided in the prompt, but the discussion directly references a recent breakdown initially reported by WindowsLatest regarding the 2024 Copilot-for-Windows app structure.*

### MemPalace, the highest-scoring AI memory system ever benchmarked

#### [Submission URL](https://github.com/milla-jovovich/mempalace) | 57 points | by [rochoa](https://news.ycombinator.com/user?id=rochoa) | [8 comments](https://news.ycombinator.com/item?id=47672792)

MemPalace: local, verbatim AI memory with a record LongMemEval score — and a candid post-launch correction

What it is
- An open-source, local-only “memory palace” for AI that stores every word of your conversations verbatim in ChromaDB, then makes them findable via semantic search.
- Organizes memory like a palace: wings (people/projects), halls (memory types), rooms (specific ideas). No LLM decides what to keep; structure + metadata filtering guide retrieval.
- Designed to run entirely on your machine with no external APIs or cloud.

Why it matters
- Most AI “memory” relies on lossy summaries and heuristics about what to keep. MemPalace’s raw mode keeps everything and still retrieves effectively, promising stronger recall, reproducibility, and privacy.

Benchmarks
- LongMemEval: 96.6% R@5 in raw mode, 500/500 questions, zero API calls; independently reproduced (M2 Ultra, <5 minutes).
- Experimental AAAK compression: currently regresses to 84.2% R@5 vs raw; readable by any LLM but trades fidelity for token density.
- Maintainers say a Haiku rerank pipeline hit 100%, but that pipeline isn’t yet in the public benchmark scripts.

Big mea culpa (April 7, 2026)
- Corrected README claims within hours of launch:
  - Token-count example for AAAK was wrong; small-scale “savings” don’t hold.
  - “30x lossless compression” was overstated; AAAK is lossy and underperforms raw mode today.
  - “+34% palace boost” was just standard metadata filtering, not a novel retrieval trick.
  - “Contradiction detection” exists as a utility but isn’t wired into the KG ops yet.
- Roadmap fixes: rewrite AAAK docs with real tokenizer counts, clearly label benchmark modes (raw/aaak/rooms), integrate contradiction checks, pin ChromaDB versions, patch a hooks shell-injection issue, and address a macOS ARM64 segfault.

Takeaways
- If you need durable, private agent memory today: use raw mode; skip AAAK for now.
- Strong transparency from maintainers; rapid iteration and clear trade-off framing.
- Worth watching as they wire in validation tools and stabilize the stack.

Here is a summary of the Hacker News discussion regarding MemPalace:

**The Celebrity Surprise**
The most immediate reaction in the thread was sheer surprise regarding the project’s author. Yes, it is *that* Milla Jovovich (the actress known for *Resident Evil* and *The Fifth Element*). Commenters were delighted by this unexpected pivot into open-source AI engineering, with one user noting it "definitely wasn't on my 2026 Bingo card," and another pointing out a missed opportunity to name the project "Resident Eval." 

**Heavy Skepticism Over Benchmark Claims**
Despite the "mea culpa" in the project's README, the community remained highly critical of the initial claims, particularly the touted 100% LongMemEval score. Commenters dug into the methodology and accused the project of "engineering benchmark results for a market breakthrough." Critics pointed out that hitting 100% involved highly targeted fixes for failing questions and aggressive LLM reranking that masked true retrieval performance (estimating an honest top-10 rerank score is closer to 88.9%). One commenter compared the marketing to previous AI hype cycles, stating it gave off "Devin/Pinocchio energy"—impressive underlying tech, but heavily asterisked methodology.

**Defending the Core Utility**
Despite the marketing flubs and README controversy, users who actually dug into the code defended the project's practical value. The independent benchmarker who ran the tests chimed in to offer a "silver lining," acknowledging that while it is entirely reasonable to bash the flawed benchmarks, a genuinely useful tool remains underneath. Several users noted that having an MIT-licensed, completely local, API-free ingest pipeline for ChatGPT/Claude chat histories—paired with a standalone temporal knowledge graph—is highly valuable. Developers are already planning to integrate MemPalace into other local agent runtimes and orchestration tools.

**Tech Concepts and Alternatives**
On a technical level, some commenters expressed distinct interest in the project's attempt at "compressing meaning using language only" (the AAAK compression), noting it's a fascinating narrative-oriented approach compared to standard auto-encoders, even if currently lossy. Finally, in standard Hacker News fashion, the thread featured developers plugging their own alternative open-source agent memory projects—coupled with a bit of classic HN snark when one of those alternative URLs contained a spelling error. 

**Overall Sentiment:** 
A highly unusual, high-profile launch that shot itself in the foot with overly aggressive benchmark marketing. However, the Hacker News community largely forgives the overexuberance because the underlying open-source code provides a genuinely useful, privacy-first local memory stack for AI agents.
...
### **Today's Top Story: Claude Code Blocked for Windows Users**
**The News:** A highly upvoted GitHub issue in the `anthropics/claude-code` repository highlights a breaking bug for Windows users. Attempting to log into Claude Code via Google OAuth results in a consistent `timeout of 15000ms exceeded` error upon returning to the app from the browser. Tested on Windows with WSL and Claude Code v2.1.92, the bug reproduces on every attempt, effectively bricking the tool for these users. No workaround currently exists.
