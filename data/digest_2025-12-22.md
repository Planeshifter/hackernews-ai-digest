...
#### [Submission URL](https://eiratansey.com/2025/12/20/i-announced-my-divorce-on-instagram-and-then-ai-impersonated-me/) | 161 points | by [robin_reala](https://news.ycombinator.com/user?id=robin_reala) | [189 comments](https://news.ycombinator.com/item?id=46352004)

I announced my divorce on Instagram and then AI impersonated me

- A writer/archivist posted a personal divorce announcement on Instagram, then shared the link to Mastodon (glammr.us). In the Toot app, her post appeared with extra paragraphs she never wrote, tacked on after her “WATCH THIS SPACE” sign‑off.

- The added copy read like generic AI self-help (“In this post, I share my personal journey…”), written in first person as if by her. It wasn’t in her Instagram caption.

- Root cause: Instagram injects an AI-generated “description” plus a pile of SEO keywords into the post’s HTML. Third-party clients that unfurl links can scrape and display that text, making it look like part of the author’s post. Viewing the IG page source shows the auto-written blurb and keywords (“self-discovery,” “finding happiness,” etc.).

- She notes Meta asks users to label AI content, but doesn’t disclose or label AI text it adds itself—nor does it seek review/consent. She calls it a violation of authorial voice and a form of impersonation. She cites earlier 404 Media reporting that Instagram is doing AI SEO bait.

Why it matters

- Silent, first-person AI “summaries” attached to user posts erode consent, provenance, and trust—especially when they leak into other platforms (e.g., the fediverse) and get misattributed.

- It blurs lines for archivists, journalists, and researchers who rely on accurate records and quotes; it may raise consumer-protection and impersonation concerns.

- Practical takeaway: developers should treat IG metadata with caution in link previews; users may want to disable previews or share screenshots when authenticity matters.
...
**The Story**
A writer and archivist recently announced her divorce on Instagram. However, when she shared the link to the post on Mastodon, the preview card featured text she did not write. The text—a first-person, AI-generated blurb about a "journey of self-discovery" and "finding happiness"—was injected by Meta into the post’s HTML metadata for SEO purposes. While the text does not appear in the Instagram caption itself, third-party clients that unfurl links scrape this hidden "description" tag, presenting it as the author’s own voice.

**The Privacy & Consent Issue**
The author argues this is a significant violation of consent and authorial voice. While Meta requires users to label their own AI-generated content, the platform silently adds AI summaries to user posts without opt-in or review. This creates a "hallucinated" reality that travels across the web, blurring the lines for archivists and journalists who rely on digital records for accuracy. It essentially forces users to "puppet" generic AI self-help advice they never gave.

**Discussion on Hacker News**
The discussion thread expands from this specific incident to a broader critique of the "Dead Internet Theory," the futility of user agency on modern platforms, and the consequences of data persistence.

*   **The Dead Internet & Vanity Metrics:** Several users argue that social media has shifted its focus from connecting humans to satisfying shareholders with "vanity metrics." One commenter shared an anecdote from a comparison shopping startup, noting that eventually, bots scraping bots outnumbered real human traffic, a precursor to today's ecosystem where AI interprets content for other AI. Users suspect that platforms no longer care if humans are interacting, provided the engagement numbers (inflated by bots and AI) remain high.
*   **Algorithmic Hostility & Dark Patterns:** The conversation drifted toward the difficulty of controlling one's digital footprint. Users shared frustrations about "sticky" algorithms (specifically TikTok) that interpret any interaction—even negative ones—as positive engagement. Others noted how difficult it is to delete accounts, citing "growth hacks" where platforms require phone numbers to delete accounts created with just emails, effectively trapping data.
*   **Data Poisoning vs. Deletion:** A debate emerged regarding "data poisoning"—intentionally feeding platforms false information—versus account deletion. While some see poisoning as a way to mess with scrapers, others warned of real-world consequences (e.g., credit scores, background checks, Palantir-style data aggregation) if verifyifiably false data is linked to a real identity.
*   **Digital Rot vs. Permanence:** While the article fears the permanence of AI-generated lies, some commenters pointed out the irony that the "real" old web (blogs, Photobucket, forums) is largely disappearing due to link rot and server shutdowns. "I used to be searchable," one user noted, "now I no longer exist according to Google," suggesting that while AI generates noise, actual human history is being lost.
*   **The "Own Your Platform" Argument:** As is common on Hacker News, the proposed solution was a return to the open web (hosting your own blog/server or using ActivityPub), though users acknowledged the friction this causes for average people trying to stay connected with friends who remain inside the "walled gardens."
...
Toad: a unified terminal UI for agentic coding tools
...
#### [Submission URL](https://ym2132.github.io/ONNX_MLProgram_NN_exploration) | 97 points | by [Two_hands](https://news.ycombinator.com/user?id=Two_hands) | [15 comments](https://news.ycombinator.com/item?id=46350075)

ONNX Runtime on Mac GPUs may silently change your model’s predictions

What happened
- While benchmarking a trained model (“EyesOff”), the author found that ONNX Runtime (ORT) on Mac GPU produced different metrics than PyTorch CPU/MPS and ORT CPU, despite identical inputs.
- Drilling down with a minimal 1-layer MLP reproduced the issue: running via ORT’s CoreMLExecutionProvider flipped a couple of predictions around a 0.5 threshold. Confusion matrices showed 2 examples switching sides.
- Root cause: ORT’s CoreML conversion defaults to a format that casts the model to FP16. That precision drop is enough to push near-threshold probabilities over/under 0.5.

Why it matters
- This isn’t just tiny floating-point noise—precision changes can alter classification outcomes and downstream metrics.
- If you deploy ONNX models to Mac GPUs via CoreMLExecutionProvider, your model can behave differently than it does in PyTorch or ORT CPU unless you pin the precision.

The fix
- Keep FP32 on Mac GPU by forcing CoreML’s MLProgram format:
  ort.InferenceSession(
      onnx_model_path,
      providers=[("CoreMLExecutionProvider", {"ModelFormat": "MLProgram"})]
  )
- This prevents the default FP16 cast and aligns outputs with PyTorch CPU/MPS and ORT CPU.

Takeaways
- Always benchmark across providers and compare raw logits/probabilities, not just metrics.
- Explicitly set precision/provider options when targeting CoreML; defaults may trade accuracy for speed.
- Expect small numeric deltas across hardware, but verify that thresholded decisions and metrics remain stable.

**Discussion Summary**

The discussion explores the tension between performance efficiency and numerical precision within the Apple ecosystem:

*   **The Role of the Neural Engine (ANE):** Commenters point out that the primary benefit of the `CoreMLExecutionProvider` is access to the Apple Neural Engine (ANE), which improves speed and battery life but strictly relies on FP16. The author (`Two_hands`) agrees but notes that the ANE is "closed," opaque regarding supported operators, and difficult to control compared to general GPU execution.
*   **Performance vs. Accuracy:** The author justifies the default behavior, noting that in their benchmarks, FP16 calculations on Apple GPUs were up to 128x faster than FP32 for large matrix multiplications. This highlights a necessary trade-off between efficiency and the reproducibility issues raised in the article.
*   **Future Providers:** There is speculation that ONNX Runtime may eventually introduce a pure MPS (Metal Performance Shaders) provider to bypass the maintenance overhead of CoreML, though this would likely forfeit ANE benefits.
*   **Debugging Tips:** Users suggested inspecting the CoreML file via the Xcode Profiler to explicitly visualize which model layers are running on the ANE versus the GPU and to verify the actual bit depth being used.

### Google's healthcare AI made up a body part – what if doctors don't notice?

#### [Submission URL](https://www.theverge.com/health/718049/google-med-gemini-basilar-ganglia-paper-typo-hallucination) | 26 points | by [tessierashpool9](https://news.ycombinator.com/user?id=tessierashpool9) | [10 comments](https://news.ycombinator.com/item?id=46359643)

Google’s Med-Gemini “invented” a body part in a demo — and Google called it a typo

The Verge reports that Google’s healthcare AI, Med-Gemini, described an “old left basilar ganglia infarct” in a head CT example — a conflation of the basal ganglia and the basilar artery that creates an anatomy that doesn’t exist. The term appeared in Google’s 2024 Med-Gemini paper and launch blog; after neurologist Bryan Moore flagged it, Google quietly edited the blog without noting the change, while the paper remained unchanged. Google says the system spotted a real pathology but used the wrong term, calling it a misspelling. Clinicians quoted call it dangerous: “Two letters, but it’s a big deal,” says Providence’s CMIO Maulin Shah. Meanwhile, Med-Gemini’s “trusted tester” program has closed to new entrants, suggesting pilot use in real settings.

Why it matters:
- High-stakes hallucinations: Small terminology errors in medicine can invert diagnoses and treatments.
- Transparency gap: Silent edits and unchanged papers erode trust in safety claims and evals.
- Safety vs. deployment: Pilots appear to be expanding before tight guardrails are standardized.

What HN might discuss:
- Required safeguards (controlled vocabularies, anatomy ontologies, named-entity checks, structured reports with atlas links).
- Human-in-the-loop and liability when AI suggestions bias clinicians (automation bias).
- Publishing norms for AI-in-medicine: versioned demos, public errata, and third-party evaluations.

Here is a summary of the Hacker News discussion:

**Google Med-Gemini's "Basilar Ganglia" Hallucination**

*   **"Typo" vs. Hallucination:** Users strongly pushed back against Google characterizing the error as a "misspelling." Commenters argued that because computers lack biological bodies, they cannot make physical slips of the finger; rather, this was a fundamental generation error. One user condemned the "typo" explanation as "arrogance," describing it as a morally questionable way to downplay the risks of AI deployment.
*   **Data Contamination Precedents:** The discussion drew parallels to "vegetative electron microscopy," a gibberish term originally created by an OCR error that eventually contaminated training data and appeared in legitimate scientific journals. Users expressed concern that AI hallucinations could similarly validate and propagate non-existent medical terminology.
*   **Clinical Implications:** Commenters deconstructed the error (conflating *basilar* and *basal*), noting that while medical terms often sound similar (e.g., Ilium vs. Ileum), the distinction is critical. There was skepticism regarding "human-in-the-loop" safety, with one user hypothesizing a scenario where tech-savvy patients are forced to fact-check AI-reliant doctors in the emergency room.
*   **Satire:** Some users mocked the situation by inventing their own fake diagnoses (e.g., "Cloomie left Glompus") and suggesting absurdity-based treatments to highlight the danger of trusting plausible-sounding but non-existent AI outputs.
