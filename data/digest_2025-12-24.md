...
Here is a daily digest summary of the top story:

**Vibium: A one-binary, zero-setup way to let AI agents drive a real browser**

**What it is**
Vibium is a new open-source browser automation stack designed specifically for AI agents and Model Context Protocol (MCP) workflows. At its core is "Clicker," a ~10MB standalone Go binary that manages the Chrome lifecycle, proxies the modern WebDriver BiDi protocol over WebSocket, and exposes an MCP server. It is Apache-2.0 licensed.

**Why it’s interesting**
*   **Agent-First Design:** It offers native MCP integration, allowing users to generic full browser control to tools like Claude Code with a single command (`claude mcp add vibium`).
*   **Zero-Config:** `npm install vibium` handles everything, including downloading a dedicated Chrome for Testing binary to a user cache, eliminating the "driver hell" often associated with Selenium.
*   **Modern Standards:** It relies on W3C WebDriver BiDi rather than the legacy Chrome DevTools Protocol (CDP).

**Roadmap**
Currently, Vibium provides the "Act" layer (clicking, typing, navigation). Future releases plan to introduce "Retina" (a recording extension/sensing layer) and "Cortex" (a memory/navigation layer to help agents model workflows), along with Python and Java clients.
