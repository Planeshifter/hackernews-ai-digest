...
Stanford study: Law professors preferred AI answers over peers 75% of the time
...
Here is a summary of the Hacker News discussion regarding the new GitHub Copilot app, formatted for a daily digest:

**The Hacker News Reaction: GitHub's Copilot App Preview**
...
#### [Submission URL](https://research.perplexity.ai/articles/rethinking-search-as-code-generation) | 72 points | by [1zael](https://news.ycombinator.com/user?id=1zael) | [20 comments](https://news.ycombinator.com/item?id=48372547)

I’m missing the submission details. Please share the Hacker News link (or the title and article URL/text), and I’ll craft an engaging digest. 

Optional: tell me your preferences:
- Length: 2–3 sentences, short paragraph, or deeper summary
- Focus: why it matters, technical highlights, or notable HN comments
- Tone: neutral, punchy, or explanatory

Although you didn't provide the original submission title or link, based on the provided comments, we can reconstruct exactly what the Hacker News community was debating. 

Here is a daily digest summarizing the thread, written in a punchy, explanatory tone with a focus on technical highlights and notable comments.

***

### **Today on HN: The Shift to AI-Native Code Search & Query Languages**

**The Context (Inferred):** The discussion centers around a new AI-driven search tool or query paradigm (with mentions of *Sourcebot* and DuckDB) that flips traditional search on its head. Instead of pulling data to the code, it uses AI to generate scripts (like Python) that execute directly against unstructured data and codebases.

**The Hacker News Discussion:**

*   **Why not just use SQL?** A debate kicked off over why the creators didn't just use a secure SQL API. Commenters pointed out that SQL is designed for strict, tabular, relational models. When dealing with free-form documents and complex semantic queries, traditional SQL (like MySQL or Postgres) falls short, making a custom, AI-driven scripting approach necessary.
*   **Speeding up Coding Agents:** A major technical focus of the thread was how to optimize LLM coding agents (like Claude) for codebases. Users noted that sequential AI tool-calling is incredibly slow and compute-intensive. Moving toward **graph-based codebase architectures** and allowing AI to write complex, multi-stage search scripts for *parallel* execution drastically reduces API turns and optimizes the LLM's context window. 
*   **The "Inefficient Code" Support Nightmare:** User *nitwit005* raised a highly practical UX concern: if the tool relies on AI-generated Python to run queries, what happens when it writes inefficient code? It creates a customer support nightmare because users will eventually hit execution limits for code they didn't write and don't know how to optimize. 
*   **Moderator Meta-Moment:** The thread took a brief detour when HN moderator *dang* intervened to flag a user (*1zael*), issuing a stern reminder that posting AI-generated or heavily AI-edited comments violates Hacker News guidelines. *(Editor's note: This likely explains the heavily compressed, vowelless text formatting in the original prompt!)*
