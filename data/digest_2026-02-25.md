...
#### [Submission URL](https://app.teamout.com/ai) | 54 points | by [vincentalbouy](https://news.ycombinator.com/user?id=vincentalbouy) | [58 comments](https://news.ycombinator.com/item?id=47151598)

TeamOut AI: an AI-powered marketplace for planning corporate offsites and retreats. Describe your event (e.g., “30 people, 3 nights in a warm location” or “US mountain retreat for 100”), and it instantly returns matching venues, then gets you pricing fast.

Highlights
- Instant matching: An AI engine curates a tailored list of venues in seconds based on your requirements.
- Vetted inventory: Hand-selected, worldwide properties with a track record hosting corporate groups.
- Fast pricing: A “smart quote” system prompts venue partners to respond within ~24 hours on average.
- Use cases: Small to large offsites, beach/mountain retreats, budget-constrained searches (e.g., under $300/night).
- Extras: Can help surface flight options for teams.

Why it matters: Planning offsites typically involves slow back-and-forth with planners and venues; this promises consumer-like speed and transparency for corporate events by combining pre-vetted supply with automated matching and quicker quotes. Open questions likely center on inventory breadth, pricing transparency, and how flights/logistics integrate end-to-end.

**The Backend "Mess" & The Coordination Layer**
Much of the discussion focused on the difficulty of digitizing the fragmented supply of independent venues. One commenter (ldvd), with experience in hotel property management systems, noted that while big chains have standardized tech, independent hotels are often a "shit show" technologically, making them hard to aggregate without significant manual effort. The founder (vncntlby) agreed, positioning the product not just as a search engine, but as a "coordination layer" designed to manage the "structured ping-pong" of availability checks, contracts, and deposits that typically occurs via email.

**Hallucinations & Search Architecture**
Several users struck friction during live testing. One user experienced location confusion (Cambridge, UK vs. Cambridge, MA), while others found the AI recommendations inferior to standard ChatGPT when dealing with complex queries like visa restrictions for distributed teams (e.g., engineers based in Pakistan).
*   **Technical Critique:** User *jmcd* suggested the system likely needs to apply "hard constraints" (dates, capacity) as a pre-filter before running vector similarity searches to improve "prediction hygiene" and prevent the AI from recommending venues that don't actually exist or fit the criteria.
*   **Visa Logic:** The founder addressed concerns about international travel logistics, claiming the algorithm attempts to use legacy data to suggest visa-friendly locations (e.g., the Dominican Republic) for specific team compositions, though users remained skeptical of the current implementation's accuracy compared to a human agent.

**Scope of Service**
There was confusion regarding the scope of "events." Some users attempted to book single-day team activities (e.g., a dinner in London) and received hotel recommendations. The founder clarified that the current model is optimized for multi-day, overnight offsites and retreats, though support for day-only events is on the roadmap.
...
HN: Your AI coding agent is drowning in noise
...
AIs chose nukes in 95% of war-game runs, study finds
...
#### [Submission URL](https://www.tolans.com/relay/how-we-hire-engineers-when-ai-writes-our-code) | 35 points | by [dfed](https://news.ycombinator.com/user?id=dfed) | [3 comments](https://news.ycombinator.com/item?id=47158939)

Headline: How we interview when AI writes most of the code

TL;DR: Tolan/Portola redesigned its engineering interview to embrace AI. Candidates build a small, real feature in a few hours using LLMs if they want, and are evaluated on judgment, architecture, clarity, and ownership—not on leetcode trivia or typing speed.

Key points
- Ditches algorithm quizzes: The author (ex-Airbnb) argues trivia-style interviews are low-signal; the loop focuses on practical, maintainable solutions.
- Real-world exercise: Candidates get a bare-bones Figma/spec for a feature that would normally take a day or two; they have a morning to produce a working slice.
- AI encouraged, ownership required: Company provides Claude/Codex/Cursor/Gemini if needed. Using LLMs is expected, but candidates must architect themselves and fully understand every line they ship.
- What’s assessed: Ability to clarify ambiguous specs, surface edge cases, reason about trade-offs, prioritize under constraints, and decide what's “review-ready” vs “ship-ready.”
- Review conversation: A 20–30 minute debrief asks what you’d improve with more time and what must change before review or production.
- Judgement over output: Working code isn’t the finish line; submitting code you don’t understand is a hard no. Creativity and user empathy are big pluses (e.g., adding a mini-game to soften LLM wait time).
- Philosophy: AI compresses implementation time, but great engineers are still defined by reasoning, communication, and taste.

Why it matters
- Reflects a broader shift: As AI lowers the cost of implementation, interviews that mirror real workflows and test product/architecture judgment may outcompete algorithm drills.
- Raises good debate for HN: Is AI-inclusive interviewing fair across experience levels? Does it privilege tool fluency or genuine design skill? How to ensure candidates aren’t outsourcing thinking?

Hiring note: They’re recruiting backend and client engineers; the author invites direct outreach.

**Discussion Summary:**

Comments on the post are generally supportive of the shift away from LeetCode, though discussion drifts into critiques of the company's product and potential biases in the new process.

*   **Support & Strategy:** User **gplv** enthusiastically endorses the approach as a better "litmus test" for modern engineering. They liken the ideal candidate to "Tony Stark"—someone whose power is amplified by their tools. To ensure candidates aren't just copy-pasting without understanding, **gplv** suggests introducing minor requirement changes mid-interview or "pulling the rug" on business assumptions to see if the candidate can adapt the LLM-generated code on the fly.
*   **Potential Bias:** User **lmsthr** offers a succinct critique, suggesting this interview style might be "personality favored," potentially prioritizing communication style over raw technical capability compared to traditional methods.
*   **Product Criticism:** In a classic Hacker News tangent, **ntmwlsn** critiques the company's underlying business (AI for families/parenting), labeling it a "weird product" and a "vile application of AI" that shouldn't replace human relationships.
...
Pentagon pressures Anthropic to relax Claude’s safety guardrails
...
#### [Submission URL](https://www.astralcodexten.com/p/the-pentagon-threatens-anthropic) | 181 points | by [lukeplato](https://news.ycombinator.com/user?id=lukeplato) | [115 comments](https://news.ycombinator.com/item?id=47154983)

Astral Codex Ten: “The Pentagon Threatens Anthropic”

- The claim: Scott Alexander reports that Anthropic’s summer 2025 Pentagon contract initially bound DoD to Anthropic’s normal Usage Policy. In January, the Pentagon allegedly pushed to change that to “all lawful purposes.” Anthropic asked for guarantees against AI-enabled mass surveillance of Americans and fully autonomous lethal use (“no-human-in-the-loop killbots”); DoD refused and, per the post, threatened consequences if Anthropic wouldn’t accept the new terms.

- The alleged “consequences”:
  - Cancel the contract.
  - Invoke the Defense Production Act to compel compliance.
  - Designate Anthropic a “supply chain risk,” which Alexander says would bar US companies using Anthropic from doing business with the military—functionally freezing Anthropic out of large swaths of the market. He calls such a move against a domestic firm unprecedented (previously aimed at foreign vendors like Huawei).

- Author’s stance: Strongly pro-Anthropic. Alexander argues DoD should pick a different vendor (e.g., OpenAI or Google DeepMind) if it needs unconstrained use, rather than coercing a contractor post-signature. He frames Anthropic’s resistance as rare principled behavior from a major AI lab and warns that weaponizing “supply chain risk” domestically is an alarming precedent.

- Counterpoints addressed: 
  - “Pentagon needs flexibility” — then don’t sign restrictive terms, switch vendors instead.
  - “Civic duty to support the military” — freedom of contract/conscience applies unless a clear legal compulsion exists; if compelled, comply or accept civil disobedience consequences.
  - “National security” — multiple AI suppliers exist; integration pain isn’t grounds for coercion. He references an official (Hegseth) and suggests ego/integration friction may be driving the standoff.

- Why it matters: Tests whether AI vendors can maintain safety and civil-liberties guardrails in government work, and whether procurement/leverage tools like DPA or supply-chain designations will be used as negotiating bludgeons against domestic AI companies.

Note: This summary reflects Alexander’s account; the Pentagon’s perspective and specific contractual details weren’t included in the post.

The discussion on Hacker News focuses on the ethical contradictions of defense contracting, the specific legal leverage available to the Pentagon, and the technical feasibility of the military’s demands.

**The "Naïve Contractor" Critique**
A prevailing sentiment in the thread is skepticism regarding Anthropic’s shock. Commenters referenced the "Torment Nexus" meme, mocking the idea of a techno-futurist company building tools capable of mass surveillance and autonomous warfare, only to be surprised when the "Department of War" insists on using them for those exact purposes. Some users argued that by accepting a defense contract in the first place, Anthropic had already crossed the Rubicon.
*   One commenter suggested a cynical "4D chess" theory: Anthropic management might secretly desire the removal of safety limitations to broaden their market but needs the Pentagon to play the "bad guy" so they can maintain their "safety-first" public image (i.e., "We didn't want to, but the government forced us").

**Government Leverage and The Defense Production Act**
There was significant debate regarding the mechanisms of coercion:
*   **The Threats:** Users analyzed the threat of the *Defense Production Act* (DPA). Some viewed the threat of designating Anthropic a "supply chain risk" as an unprecedented weaponization of national security tools against a domestic company to win a contract negotiation.
*   **The Outcome:** Several users predicted this is a negotiation bluff. One argued that despite the high-stakes rhetoric, the most likely outcome is a "face-saving compromise" because canceling a $200M contract over guardrails—or attempting to nationalize the model—would be politically messy and legally complex.
*   **Standard Operating Procedure:** While the article frames this as specific to the current administration (references to Pete Hegseth and "alcoholic neighbor" metaphors), others argued that the US government has a long history of "strong-arming" contractors and that both major political parties generally align on expanding the defense establishment's powers (citing FISA courts).

**Technical Feasibility: "Killbots" vs. Hallucinations**
A sub-thread debated the technical reality of "autonomous lethal use."
*   **Existing Autonomy:** Using the Patriot missile system as an example, users pointed out that automated lethal engagement rules already exist in the military.
*   **LLM Limitations:** Counter-arguments distinguished between deterministic heuristic systems (like missile defense) and Transformer-based LLMs. Users argued that putting an LLM—which is prone to "hallucinations" and unpredictability—"in the loop" for lethal decisions is a fundamentally different and more dangerous proposition than existing automated weaponry.
*   **Model Weights:** One user speculated on *how* the government would achieve its goals, noting that simply removing contractual restrictions doesn't change the model weights (the safety training). The government would essentially get "Claude-without-guardrails," but effective implementation would require retraining or fine-tuning, which implies the government might seek to "nationalize" the model to modify it directly.
