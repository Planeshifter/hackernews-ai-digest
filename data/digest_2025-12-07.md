...
Bag of words, have mercy on us (Experimental History by Adam Mastroianni)
...
**Top Story: Oxide’s Values-First Playbook for LLMs**

Oxide Computer Company’s Bryan Cantrill has outlined a "values-first" rubric for utilizing Large Language Models (LLMs), rejecting static policies in favor of dynamic judgment based on company principles. The priority framework emphasizes **Responsibility** (humans retain full accountability), **Rigor** (LLMs must sharpen, not replace, thinking), **Empathy** (remembering the human audience), **Teamwork** (avoiding the "AI-generated" disclosure as a distancing mechanism), and finally **Urgency**.

Cantrill argues LLMs excel as readers (summarization, comprehension) and editors (structure, phrasing), but perform poorly as primary writers, often producing cliché-ridden text that erodes trust. The playbook warns against using LLMs to bypass socially expected reading duties, such as evaluating job candidates, and ultimately positions the technology as a tool for critique and comprehension rather than a substitute for human voice and judgment.
