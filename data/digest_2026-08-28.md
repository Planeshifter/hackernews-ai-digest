...
#### [Submission URL](https://groverlab.org/hnbfpr/2026-08-26-ai-counterfeit-cosmetics.html) | 21 points | by [wgrover](https://news.ycombinator.com/user?id=wgrover) | [9 comments](https://news.ycombinator.com/item?id=49484925)

**Gemini 3.6 Flash, given six images per item with “Thinking” enabled, correctly flagged a $5 eBay “Rhode Peptide Lip Tint” as counterfeit**, surfacing concrete tells while also tripping on glare-induced misreads.

- Setup: three packages (A/B/C); for each, four box sides plus tube front/back were uploaded; prompts were “Is this tube … authentic?” then “Any other red flags?”
- What it caught on Package A (fake): 
  - Typo in ingredients (“Diisosteary! Malate”) consistent with sloppy OCR on the box — a real red flag.
  - Cross-photo inconsistency in the Responsible Person: BIORIUS on the box vs PWC Services on the tube; the author notes their authentic tubes list BIORIUS only, so PWC on recent stock suggests a counterfeit.
  - A recurring crimp-stamped batch code “112505” seen on multiple counterfeits the author purchased — avoid if you see it.
- Where it failed: it hallucinated a misspelling (“SOLOTIONS”) by mistaking glare for a printing error; it did, however, validly note a missing space (“LIMITED,STANLAW”).

The takeaway is pragmatic: off-the-shelf vision LLMs can spot cross-image inconsistencies and OCR-like typos that humans might miss, but they’re brittle to photographic artifacts — lighting and image quality matter if you want this to be useful at point of purchase. The post frames this against a backdrop where studies estimate two-thirds of branded cosmetics on popular marketplaces are fake, so even imperfect automation could meaningfully help.

The discussion centered on the mechanics of the packaging errors that the AI caught. When asked why counterfeiters don't just duplicate the text perfectly, commenters explained the production reality: lacking original vector files, counterfeiters run OCR on scanned packages to recreate crisp text for printing, inadvertently baking in OCR typos. They avoid spending the extra money on a graphic designer to proofread simply because the flawed versions already succeed in fooling most buyers.

Other thread highlights:
- **The inevitable arms race:** Commenters noted that if AI detection scales and starts impacting sales, counterfeiters will just use the same vision LLMs as a QA step to perfect their knockoffs.
- **Broader applications:** Users reported existing success using AI to spot fakes in fast-paced live auctions, and suggested laptop batteries as another market plagued by easily spotted counterfeits.
- **Author's background:** The community surfaced other creative, low-tech physical security projects from the author's lab (Grover Lab), such as using candy as cryptographic tags.
...
#### [Submission URL](https://www.ubu.com/historical/racter/index.html) | 9 points | by [buescher](https://news.ycombinator.com/user?id=buescher) | [3 comments](https://news.ycombinator.com/item?id=49483622)

**With the exception of its introduction, this 1984 book claims every page was written by a computer**, then proofread only for spelling. The system, Racter, ran as compiled BASIC on a Z80 micro with 64K of RAM and generated prose by combining categorized word lists with “syntax directives” — rule-like templates that select from piles of words to form grammatical sentences. It conjugates regular and irregular verbs, handles singular/plural forms, tracks noun gender, and can assign variable status to randomly chosen “things” (from words up through whole story forms). By reusing these variables throughout a passage, it threads recurring elements that read as coherence — “crazy thinking,” the introduction says, but in correct English.

The thesis is explicit: prioritize grammatical correctness and novelty over human-grounded meaning to yield text that can be fascinating, humorous, even aesthetically pleasing. The name “Racter” was a six-character foreshortening of raconteur, forced by filename limits on the original machine. UbuWeb hosts the PDF of The Policeman’s Beard Is Half-Constructed; the original book’s collage illustrations are not included in this edition.

The brief thread consists entirely of supplemental resources for exploring Racter directly. Commenters pointed to an MS-DOS emulator and a digital scan of the original illustrated book—both hosted on the Internet Archive—while noting the system's early appearance in Douglas Hofstadter’s *Metamagical Themas*.
...
#### [Submission URL](https://github.com/sseshachala/conductai) | 20 points | by [sudhendra1](https://news.ycombinator.com/user?id=sudhendra1) | [3 comments](https://news.ycombinator.com/item?id=49483173)

**Pre-execution policy enforcement across LLM and shell/MCP actions, with signed configs and a SHA-256 hash-chained audit log,** moves this from “observability” to governance—and defaults to fail-closed.

- Two surfaces, one policy:
  - **Guard** — policy engine that can block/warn/audit/inject before any AI action runs, verifying a workspace-signed policy pack and appending every decision to a hash chain.
  - **Router** — LLM proxy; point Anthropic/OpenAI/Perplexity SDKs at it and every call flows through Guard on the way upstream.
- “Runtime firewalls like Straiker and Lakera tell you what an agent did.” This controls what an agent can do, with cryptographic proof and one-click audit verification.
- Lens chat UI sits above your workspace data (audit log, policy state, run history) and is Guard-enforced; ask “who got blocked today” and drill into full audit records.
- Discovery mode is the wedge: 14 days of read-only visibility into every AI action, no upstream installs, no cost; promote rules directly from what Discovery observes.
- Quick start: docker compose up brings up API (:8000 with /guard/* and /proxy/*), Canvas UI (:3000), Redis, and Postgres; curl the Router or wrap your CLI.
- CLI hooks install MCP and pull policies so Claude Code, Cursor, Copilot, ChatGPT, and Codex sessions on that machine share the same active packs.
- Ships with 20+ compliance packs (OWASP, SOC 2 CC7.3, HIPAA §164.312, PCI DSS 4.0, EU AI Act Art. 15/16, NIST AI RMF, ISO 42001, plus Python/Node/Terraform) and 22 YAML playbooks (Issue→PR, code review, incident response, prod deploy gates, CI/CD and security triage, Slack digests).
- Coverage spans LLM and shell/MCP; config integrity is verified on every check; audit evidence is tamper-evident and verifiable.

The standout differentiators are the fail-closed default, signed policy packs, and hash-chained audits—governance primitives you can hand to an auditor, not just logs after the fact.

Commenters bypassed the project's specific architecture to share alternative approaches to agent guardrails, highlighting **agentjail** (OPA-backed policies with an OS-native sandbox) and the Rust crate **coding-tools** (a lighter-weight option focused strictly on harness tool calls).
