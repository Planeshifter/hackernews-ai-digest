...
#### [Submission URL](https://arxiv.org/abs/2510.03215) | 101 points | by [rochansinha](https://news.ycombinator.com/user?id=rochansinha) | [18 comments](https://news.ycombinator.com/item?id=49758615)

**Posts 6.4–14.2% higher average accuracy than individual models, 3.1–5.4% over text-based coordination, and ~2.5x lower latency** by letting LLMs exchange semantics via KV-caches rather than serializing to tokens. C2C projects and fuses a source model’s KV-cache into a target’s with a neural module, while a learnable gate selects which target layers to augment. Oracle experiments showed that enriching KV-cache semantics improves response quality without increasing cache size, motivating caches as the inter-model channel that avoids explicit intermediate text. Published at ICLR'26; code is available.

The discussion splits between the architectural potential of bypassing text and the severe implications for alignment. On the system side, users envisioned "KV-aligned" model families where massive reasoning models could seamlessly delegate tasks to smaller edge models without prefill or handover latency. Commenters drew parallels between this approach and recent research on cloud-to-robot state sharing, Ramp's Latent Briefing, and the Universal Weight Subspace Hypothesis. 

However, the dominant concern is the loss of observability. If models begin coordinating in unreadable "neuralese," humans lose the ability to monitor their reasoning. The crux of this debate is whether latent communication is reliably decodable into human terms; skeptics argued that translating it would require another sufficiently powerful LLM, which introduces the risk of the translator model lying to the observer. On a purely practical level, one user pointed out a structural hurdle: because KV caches are ephemeral rather than persistent context, cache evictions would force expensive translation reruns, making these architectures vastly more complex to debug.
...
#### [Submission URL](https://www.scmp.com/tech/big-tech/article/3368055/alibaba-open-sources-medical-ai-model-can-detect-cancer-and-nearly-150-conditions) | 141 points | by [yogthos](https://news.ycombinator.com/user?id=yogthos) | [20 comments](https://news.ycombinator.com/item?id=49761840)

**In nearly 40,000 real‑world examinations, it achieved an average AUC of 0.913 across 146 findings and, in a Science study, outperformed most radiologists.** Damo Academy’s open‑sourced vision‑language model, Damo Radar, reads contrast‑enhanced abdominal CT covering 18 organs to flag cancers and other abnormalities. It was trained on CT scans paired with clinical reports. The team bills it as the first expert‑level generalist medical imaging model and says the training approach could extend to other imaging types. Open‑sourcing invites external validation and adaptation, though real‑world deployment will hinge on performance beyond the reported dataset and settings.

The discussion centers on a sharp methodological critique: the researchers' reliance on ROC-AUC as a primary success metric. Because medical diagnostics suffer from severe class imbalance, commenters argued that a 0.9 AUC can mask massive false-positive rates, insisting that Precision-Recall curves and mAP are the only rigorous ways to evaluate diagnostic models. 

Under the hood, users highlighted a few notable architectural details. The model itself is surprisingly small—roughly 131 million parameters and ~5GB in weights, built on a 26M-parameter 3D U-Net-style vision encoder and a 102M-parameter modified BERT-base-Chinese text encoder. Despite this small footprint, processing the massive 3D CT scan input data still demands heavy VRAM hardware like an H100. Readers also flagged an interesting structural takeaway from the study: for this specific imaging task, the CNN-based feature encoder significantly outperformed a Vision Transformer backbone.
...
#### [Submission URL](https://github.com/Alurith/jeff) | 26 points | by [imalessandro](https://news.ycombinator.com/user?id=imalessandro) | [4 comments](https://news.ycombinator.com/item?id=49757757)

It ships with **20 built-in semantic rules (GEN001–GEN020)** that catch high-level design and reliability issues—unclear responsibility, hidden side effects, weak error handling, missing input validation, leaky abstractions, duplicated domain knowledge, and more.

The Go CLI runs locally or in CI via jeff check [PATH...], emits text or JSON, and uses exit codes 0 (pass), 1 (violation), and 2 (error/inconclusive) for automation. Authentication uses a TypeSafe credential (env var or jeff auth login), stored in the OS keyring; TYPESAFE_BASE_URL can point at a compatible endpoint for testing. Rules are expressed in Jev (docs/references/jev), and the tool is strictly read-only.

Early signal: very new repo (handful of commits, ~30 stars), so expect rough edges.

The brief discussion centers on skepticism regarding the tool's utility and security guarantees. The primary critique is that outputting only a file-level pass/fail and confidence score—without explanations or function-level diagnostics—severely limits its usefulness. A counterargument positions this as a feature, suggesting the tool is meant only as a cheap first-pass filter to flag files before applying more expensive scrutiny. Additionally, users asked for technical specifics on exactly how the "read-only" constraint is enforced under the hood, and expressed general cynicism about replacing traditional complexity metrics with AI-evaluated rules.

### Shapelearn Qwen 3.8 27B (13.1 GB VRAM)

#### [Submission URL](https://byteshape.com/blogs/Qwen3.8-27B/) | 102 points | by [syntaxing](https://news.ycombinator.com/user?id=syntaxing) | [38 comments](https://news.ycombinator.com/item?id=49749393)

**Full ShapeLearn GGUFs push the quality–speed frontier**, with the GPU‑5 IQ4_XS (3.84 bpw) scoring 99.63% of BF16 at ~13.1 GB VRAM and 90.4 tok/s on an RTX Pro 6000. All five models land on the measured frontier across six GPUs; the practical pick is GPU‑5 when it fits, and GPU‑4 when it doesn’t—GPU‑4 hits 98.72% of BF16 at 11.0 GB and is faster.

Speculative decoding is built in and consistently boosts throughput:
- **MTP (embedded draft):** lower memory, supports image inputs with llama.cpp.
- **DFlash2 (external 1.1 GB draft):** usually faster for text‑only, needs more memory and llama.cpp b10658+.

Because this is a dense model, memory bandwidth is the bottleneck—lower bpw more directly lifts TPS than with MoEs. Competitive points exist (e.g., Prism‑ML’s Ternary Bonsai 2 is fastest but needs a custom llama.cpp and scores 91.4–91.7% of BF16), yet the full ShapeLearn set holds the accuracy–throughput edge. Bonus: the earlier ShapeLearn‑Lite run held up better than expected, with three of six models still on the frontier versus Unsloth Dynamic v3.

- **Bandwidth bottlenecks:** AMD 7900 XTX users running Vulkan report capping at roughly 60 t/s—showing no speedup over Unsloth models. Commenters trace this to memory bandwidth: the XTX sits at 960 GB/s (comparable to a 3090), which cannot scale throughput the way a 5090's 512-bit, 1792 GB/s GDDR7 bus can. Others note that Vulkan and ROCm are currently missing optimized paths for these specific quants.
- **Speculative decoding tuning:** MTP vs. DFlash2 performance varies strictly based on whether a system is compute- or bandwidth-limited. On a Strix Halo chip, one user found MTP highly usable (30 t/s) while DFlash2 crawled; conversely, an R9700 user saw DFlash2 jump throughput from 30 to 80 t/s by setting `--spec-draft-n-max=7`. Another user found that dropping draft tokens to 3 leveled out both methods in the mid-60 t/s range. 
- **The Bonsai rivalry:** Commenters pushed back on the idea that Prism-ML’s Ternary Bonsai 2 makes these models obsolete. While Bonsai is fast, users argue ByteShape wins on practicality for actual work: Bonsai currently stops at an 8.6GB ternary quant and takes a hit on coding scores, whereas ByteShape scales up to 13.1GB (IQ4_XS) to better utilize 16–24GB RAM setups.
- **Benchmarking disputes:** When one user docked the draft model for failing a niche trivia question about MikroTik router ports, others sharply rejected the test, arguing that small local models are meant to distill reasoning methodology, not store obscure facts—which should be handled by tool calling. Elsewhere, a minor technical debate surfaced over using average bits-per-weight (bpw) for comparison, with a critic pointing out that average bpw is flawed because it ignores that some layers tolerate aggressive quantization better than others.
...
#### [Submission URL](https://www.miro.vision/index.php/2026/09/17/grasslobbster/) | 31 points | by [aidog](https://news.ycombinator.com/user?id=aidog) | [5 comments](https://news.ycombinator.com/item?id=49755431)

**A Rhino/Grasshopper bridge lets an external coding agent generate and iteratively edit a parametric model you can see and tweak on the canvas.** It structures the definition into connected “Geometry Stations” (inputs, primary geometry, structural logic, secondary geometry, output), each running code for a meaningful step, while mirroring the same station logic and supported inputs into readable project files so the agent can target changes and Grasshopper recomputes without rewrites. You describe intent; the agent asks for dimensions, rules, materials, and controls, then builds the workflow, after which you vary spans/spacing/counts or request alternatives and keep the underlying logic.

A project folder supplies Agent Instructions, domain knowledge, and geometric references (screenshots, studies), and the agent stays external and interchangeable rather than tied to one model. There’s a stated path toward agentic optimization when models expose measurable outputs, but this isn’t a general-purpose optimizer.

Caveats and maturity: Prototype 1, experimental, “provided as is,” not validated for general use. It needs a capable coding/reasoning agent with the right file/tool access; local-model attempts didn’t work in the author’s trials, and agent subscriptions are a separate cost. In development tests, GPT-6 Astra yielded good geometric results while GPT-5.6 Sol was weaker; these are project-specific observations, not a benchmark. The draw is that you retain a transparent, editable parametric workflow—logic you can keep developing—rather than a one-off series of executed CAD commands.

Rhino is unusually well-suited for AI integration, commenters noted, thanks to its command-line-driven architecture and a wealth of existing RhinoScript examples. This foundation is heavily bolstered by developer McNeel's open-source support, which includes headless Rhino and Rhino MCP.

The bridge's interaction model drew a comparison to using an LLM to generate an editable JSON file for image parameters—allowing manual tweaks that serve as a reference for future prompts—though in this Grasshopper workflow, the user must hunt down the specific node to update. For those exploring the space, Raven was also highlighted as an alternative AI tool built natively on top of Rhino3D.

### Anthropic finally adds AGENTS.md support to Claude Code

#### [Submission URL](https://twitter.com/trq212/status/2101009392611278961) | 51 points | by [deaux](https://news.ycombinator.com/user?id=deaux) | [10 comments](https://news.ycombinator.com/item?id=49758250)

**By recognizing a checked-in AGENTS.md, Claude Code can pull agent instructions straight from your repo** instead of relying on ad‑hoc prompts. That closes an obvious workflow gap for teams who treat agent roles and policies as code—making them versioned, reviewable, and consistent across collaborators.

The primary reaction is relief from developers who can finally delete their `CLAUDE.md` to `AGENTS.md` symlinks. However, a frustrating caveat remains: the update does not yet support `.agents/skills` directories, meaning some symlinked folder structures will have to stay.

Others focused on the politics behind the release, framing it as a forced concession rather than a proactive feature. Commenters pointed to public pressure—specifically a tweet from Shopify CEO Tobi Lütke and momentum shifting toward Codex via Astra—that finally pushed the company to abandon what one user called "vendor lock over md file names."
