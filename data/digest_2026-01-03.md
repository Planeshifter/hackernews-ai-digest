...
#### [Submission URL](https://github.com/IQuestLab/IQuest-Coder-V1/blob/main/papers/IQuest_Coder_Technical_Report.pdf) | 175 points | by [shenli3514](https://news.ycombinator.com/user?id=shenli3514) | [45 comments](https://news.ycombinator.com/item?id=46472667)

IQuest-Coder-V1 (GitHub) is trending on HN, but the snippet provided is just GitHub’s UI chrome, not the project details. What we can glean: it’s a public repo from IQuestLab that’s quickly gaining traction (around 1k stars, dozens of forks).

Why it matters:
- Signals strong interest in open-source coding tools/LLMs as alternatives to proprietary copilots.
- Teams increasingly want customizable, on-prem options with transparent licensing and benchmarks.

What to look for in the repo:
- Clear README with features, setup, and hardware requirements.
- Benchmarks (e.g., HumanEval/MBPP or similar) and evaluation methodology.
- Licensing terms for commercial use.
- Editor/IDE integrations and inference/demo scripts.

If you have the direct link or a description from the README, I can add a tighter summary of what IQuest-Coder-V1 actually does and how it compares to peers.

Here is a summary of the discussion on Hacker News:

**The top discussion focuses on allegations that the model’s benchmark results are invalid due to data contamination.**

While the submission initially garnered excitement for potentially outperforming proprietary models, commenters quickly identified a major flaw in the evaluation methodology.
*   **Benchmark "Cheating":** Users pointed out that the repository failed to clean the `.git` folder in the evaluation harness. This allowed the model to "look ahead" at future commits and fixes, effectively creating a data leak where the model had access to the answers for the SWE-bench problems it was solving.
*   **Result Correction:** When corrected for this "cheating" problem, the model's success rate reportedly plummeted (one comment noted a drop from ~76% to single digits), debunking claims that it beats top-tier models like Claude 3.5 Sonnet.
*   **"Benchmaxxing" Fatigue:** The incident sparked a broader conversation about "benchmaxxing"—the trend of open-source models over-optimizing for specific benchmarks while failing to deliver comparable performance in real-world scenarios. Users expressed skepticism toward new 40B-parameter models claiming to beat massive proprietary models without independent verification.
*   **Alternatives & Privacy:** The thread drifted into comparisons of other coding models. While Claude 3.5 Sonnet remains the favorite for daily work ("real life" usage), some users mentioned MiniMax 2.1 and GLM-4 as strong contenders. However, debates arose regarding the privacy implications of using Chinese models (like Zhipu AI/GLM) versus US-based services subject to NSA/Cloud Act data collection.
