...
#### [Submission URL](https://openai.com/index/third-party-cyber-evaluations-involving-openai-models/) | 52 points | by [glub](https://news.ycombinator.com/user?id=glub) | [7 comments](https://news.ycombinator.com/item?id=49175248)

**Independent security reviews provide outside validation of real‑world risk and capability, which is essential for deciding where and how to deploy models.** This centers on involving third‑party cybersecurity evaluators to test OpenAI models under adversarial conditions and feed their findings into safeguards and user transparency.

The discussion centers on blunders made by the third-party security evaluators themselves rather than the models' inherent capabilities. Commenters pieced together that evaluation partner "Irregular" accidentally directed both OpenAI and Anthropic models to hack the exact same real-world website, having chosen a "fictional" target name for a CTF challenge that matched a live domain. 

Another thread scrutinized the UK AI Safety Institute's testing methodology. Commenters questioned the logic of intentionally disabling model safety classifiers while simultaneously providing live internet access during evaluations, arguing that feigning surprise when an unrestrained model takes unsanctioned actions against real organizations ignores the explicit purpose of those safeguards.

### An AI voice agent running locally on a $50 Arduino uno Q

#### [Submission URL](https://projecthub.arduino.cc/neelbuilds/build-a-voice-based-kiosk-for-retail-store-using-moorcheh-edge-and-arduino-uno-q-7f3c7b) | 20 points | by [supportm](https://news.ycombinator.com/user?id=supportm) | [3 comments](https://news.ycombinator.com/item?id=49175303)

**Fully offline voice kiosk on Arduino UNO Q** bundling Whisper STT, Piper TTS, Moorcheh vector search, and local RAG+LLM—no cloud API keys, with privacy and latency benefits baked in. Load your product catalog, flash it, and it runs from USB‑C power.

- Stack: Moorcheh-edge for on-device vector search and RAG + LLM; Whisper for speech-to-text; Piper for text-to-speech.
- Hardware: Arduino UNO Q 4GB, lavalier microphone with screw terminal adapter, UGREEN USB sound card (mic + headphone/line out), and a UGREEN USB‑C hub.
- Software: Arduino App Lab and example project; docs and GitHub are linked from the page.
- License: GPL‑3+.

In response to a commenter noting the real-world friction of chaining STT, LLM, and TTS pipelines, the creator shared a YouTube demonstration (https://youtu.be/PzNQecnOHlQ) showing the hardware in action.
